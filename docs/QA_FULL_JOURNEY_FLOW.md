...
| **ניקוי אוטומטי** | בסוף כל ריצה (afterEach) – מחיקה אוטומטית של כל הנתונים שנוצרו (לקוח, פרויקט, אירוע, קבלן, יועץ, ספק, דיווחי שעות וכו'). דורש: `E2E_SUPABASE_URL` ו־`E2E_SUPABASE_ANON_KEY` (או `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY`). אם לא הוגדרו – הנתונים נשארים. |
