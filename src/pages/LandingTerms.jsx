...
function LandingTermsContent() {
  const { t } = useLandingLanguage();
  const footerT = t('footer');
  const legalT = t('legal');
  const content = legalT?.terms;

  if (!content) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16">
        <h1 className="text-2xl font-bold text-gray-900">{footerT?.termsOfService || 'תנאי שימוש'}</h1>
        <p className="mt-4 text-gray-600">תוכן תנאי השימוש ייטען כאן.</p>
      </div>
    );
  }
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[#F7F5F2] via-white to-white pt-8 sm:pt-12 pb-16 sm:pb-24">
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6">
        ...
        >
          <FileText className="w-10 h-10 text-[#984E39]" />
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900">
            {footerT?.termsOfService || 'תנאי שימוש'}
          </h1>
        </motion.div>
